...
def modify_model(model, name='resnet50', rate=4, save=False):
    ...
    for l in conv_layers_list:
        ...
        for i in range(model.layers[l].filters):  ##kernel num
            for s in range(model.layers[l].input_shape[-1]):  # kernel depth
                i1 = index_list[s][i][0]
                i2 = index_list[s][i][1]
                x = np.array(dic_list[s][i1]).reshape(filter_size, filter_size)
                y = np.array(dic_list[s][i2]).reshape(filter_size, filter_size)
                a = a_list[s][i]
                b = b_list[s][i]
                weights_new[0][s, i] = a
                weights_new[1][s, i] = b
    ...
    if save:
        model_new.save_weights("../weights/%s_modified_weights_%d.h5" % (name, rate))
